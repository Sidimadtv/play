...
test.beforeAll(async () => {
    ...
    const isMainWindow = (await page.title()) === 'sidimad IPTV';
    if (!isMainWindow) {
        page = app.windows()[1];
    }
...
test.describe('Check Home Page', () => {
    test('Launch electron app', async () => {
        const windowState = await app.evaluate((electronProcess) => {
            ...
            const isMainWindow = mainWindow.title === 'sidimad IPTV';
            if (!isMainWindow) {
